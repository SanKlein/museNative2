...
class UserSettingsPage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.state.edit === 'changed' && loading && !nextProps.loading && editUser.name === nextProps.user.name && editUser.email === nextProps.user.email) {
      this.setState({ edit: 'saved' })
      navigator.pop(0)
    }
    if (this.state.edit === 'changed' && loading && !nextProps.loading && nextProps.user.name !== nextProps.editUser.name && nextProps.user.name !== nextProps.editUser.name) {
  ...
  handleSave() {
    let { user, editUser, saveEditUser, setError, state } = this.props
  ...
  render() {
    ...
    if (editUser.name && editUser.email && (editUser.name !== user.name || editUser.email !== user.email) && (edit === 'saved' || edit === 'changed')) {
    ...
    return (
      <Page>
        <Container>
          ...
          <TouchableOpacity style={styles.logoutButton} onPress={this.confirmLogout} activeOpacity={.7}>
...
const mapStateToProps = ({ user, editUser, error, loading }) => ({ user, editUser, error, loading })

UserSettingsPage = connect(
  mapStateToProps,
  { logout, changeEditName, changeEditEmail, saveEditUser, setError }
)(UserSettingsPage)
