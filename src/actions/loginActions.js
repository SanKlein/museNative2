...
export const changeLoginPassword = (password) => ({ type: ActionTypes.CHANGE_LOGIN_PASSWORD, password })
...
export const signupUser = (user) => (dispatch) => {
  ...
  return fetch(ActionTypes.URL + '/api/signup', { method: 'POST', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ user }) })
    .then(handleErrors)
    .then(responseJson => dispatch({ type: ActionTypes.SIGNUP_USER_SUCCESS, user }))
    .catch(error => dispatch({ type: ActionTypes.SIGNUP_USER_FAIL, error: 'Sorry, name or email is taken' }))
}
...
export const loginUser = (user) => (dispatch) => {
  ...
  return fetch(ActionTypes.URL + '/api/login', { method: 'POST', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ user }) })
    .then(handleErrors)
    .then(responseJson => {
    ...
    .catch(error => dispatch({ type: ActionTypes.LOGIN_USER_FAIL, error: 'Sorry, password does not match' }))
}
...
// cancel login before started
export const cancelLogin = () => ({ type: ActionTypes.CANCEL_LOGIN })
